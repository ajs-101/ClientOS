...
export default function EmployeeProfile() {
  ...
  useEffect(() => {
    if (activeProfile !== profileId) navigate("/employees");
  }, [activeProfile, profileId, navigate]);

  useEffect(() => {
    if (!activeOrg || !profileId) return;
    runOverdueEscalation(activeOrg);

    const tasksQ = query(collection(db, "employeeTasks"), where("orgId", "==", activeOrg), where("profile", "==", profileId));
    ...
    const activityQ = query(collection(db, "activityLog"), where("orgId", "==", activeOrg), where("profile", "==", profileId), orderBy("createdAt", "desc"));
    const unsubActivity = onSnapshot(activityQ, (snap) => setActivity(snap.docs.slice(0, 8).map((d) => ({ id: d.id, ...d.data() }))));

    return () => { unsubTasks(); unsubActivity(); };
  }, [activeOrg, profileId]);
    ...
    try {
      const res = await fetch("/.netlify/functions/profile-summary", {
        ...
        body: JSON.stringify({
          ...
          tasks: tasks.map((t) => `${t.title} [${t.status}]${t.dueDate ? ` due ${t.dueDate}` : ""}`),
        }),
      });
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.9rem" }}>
          <div style={{ width: 46, height: 46, borderRadius: 12, background: `${profileConfig.color}22`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icon size={22} color={profileConfig.color} />
          </div>
          <div>
            <h1 style={{ fontSize: "1.6rem" }}>{profileConfig.name}</h1>
            <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>{profileConfig.people}</p>
          </div>
        </div>
      ...
      <div className="glass" style={{ padding: "1.25rem 1.5rem", marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <span style={{ fontSize: "0.9rem", color: summary ? "var(--text-primary)" : "var(--text-muted)" }}>
            {summary || `AI overview of everything on ${profileConfig.name}'s plate.`}
          </span>
        </div>
      ...
      <div style={{ display: "grid", gap: "0.9rem", marginBottom: "2.5rem" }}>
        {tasks.length === 0 && <p style={{ color: "var(--text-dim)", fontSize: "0.85rem" }}>No tasks yet.</p>}
        {tasks.map((t) => (
          <TaskCard key={t.id} task={t} authorLabel={profileConfig.name} onStatusChange={handleStatusChange} onDelete={handleDeleteTask} />
        ))}
      </div>

      <div>
        <h2 style={{ fontSize: "1rem", display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.9rem" }}>
          <Activity size={16} color="var(--accent-teal-bright)" /> Recent activity
        </h2>
        <div style={{ display: "grid", gap: "0.5rem" }}>
          {activity.length === 0 && <p style={{ color: "var(--text-dim)", fontSize: "0.8rem" }}>Nothing logged yet.</p>}
          {activity.map((a) => (
            <div key={a.id} style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              {a.message} <span style={{ color: "var(--text-dim)", fontSize: "0.7rem" }}>· {timeAgo(a.createdAt)}</span>
            </div>
          ))}
        </div>
      </div>
