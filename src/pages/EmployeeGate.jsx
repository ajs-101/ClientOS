  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontSize: "1.8rem" }}>Employee Profiles</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "1.25rem" }}>
        {[...PROFILE_LIST, PROFILES.admin].map((p) => {
          ...
          return (
            <div
              key={p.id}
              onClick={() => { setSelected(p.id); setError(false); setPassword(""); }}
              className="glass glass-interactive employee-card"
              style={{
                padding: "1.75rem 1.5rem",
                cursor: "pointer",
                border: isSelected ? `1px solid ${p.color}` : undefined,
                boxShadow: isSelected ? `0 16px 40px -18px ${p.color}66` : undefined,
              }}
            >
              <div style={{
              ...
              <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginTop: "0.2rem" }}>{p.people}</p>

              {isSelected ? (
                <div style={{ display: "grid", gap: "0.6rem", marginTop: "1.25rem" }} onClick={(e) => e.stopPropagation()}>
                  <input
                    type="password"
                    autoFocus
                  ...
                  />
                  {error && <p style={{ color: "var(--danger)", fontSize: "0.72rem" }}>Incorrect password</p>}
                  <button
                    onClick={() => handleUnlock(p.id)}
                    style={{
                      background: `linear-gradient(135deg, ${p.color}, ${p.color}bb)`,
                      color: "#0A0A0A", fontWeight: 600, border: "none", borderRadius: 10, padding: "0.6rem",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginTop: "1.25rem", color: "var(--text-dim)", fontSize: "0.78rem" }}>
