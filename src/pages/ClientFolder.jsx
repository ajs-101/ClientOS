...
import { doc, getDoc, collection, addDoc, onSnapshot, query, where, orderBy, deleteDoc, updateDoc } from "firebase/firestore";
import { db } from "../lib/firebase";
import { uploadFile } from "../lib/cloudinary";
import { Upload, Sparkles, FileText, StickyNote, Plus, Trash2, Pencil, Check, X } from "lucide-react";
            ...
            {editingNoteId === n.id ? (
              <div style={{ display: "grid", gap: "0.6rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button className="btn-primary" onClick={() => saveEditNote(n.id)} style={{ display: "flex", alignItems: "center", gap: "0.3rem", padding: "0.5rem 1rem" }}>
                    <Check size={14} /> Save
                  </button>
                  <button className="btn-ghost" onClick={() => setEditingNoteId(null)} style={{ display: "flex", alignItems: "center", gap: "0.3rem", padding: "0.5rem 1rem" }}>
                    <X size={14} /> Cancel
                  </button>
                </div>
              </div>
              ...
              <>
                <div style={{ position: "absolute", top: "1rem", right: "1rem", display: "flex", gap: "0.5rem" }}>
                  <button
                    ...
                    style={{ background: "none", border: "none", color: "var(--text-dim)", display: "flex", opacity: 0.6, transition: "opacity 0.2s ease, color 0.2s ease" }}
                    onMouseEnter={(e) => { e.currentTarget.style.opacity = 1; e.currentTarget.style.color = "var(--accent-teal-bright)"; }}
                ...
                <p style={{ fontWeight: 500, marginBottom: "0.5rem", paddingRight: "3.5rem" }}>{n.title}</p>
                <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", whiteSpace: "pre-wrap", lineHeight: 1.6 }}>
                  {linkify(n.content)}
                </p>
                {n.editedAt && (
                  <p style={{ fontSize: "0.68rem", color: "var(--text-dim)", marginTop: "0.5rem" }}>Edited {new Date(n.editedAt).toLocaleString()}</p>
                )}
              </>
            )}
          </div>
          ...
          <div key={a.id} className="glass glass-interactive" style={{ padding: "1.25rem", position: "relative" }}>
            {editingAssetId === a.id ? (
              <div style={{ display: "grid", gap: "0.5rem" }} onClick={(e) => e.stopPropagation()}>
                <input value={editFileName} onChange={(e) => setEditFileName(e.target.value)} style={{ fontSize: "0.8rem", padding: "0.5rem 0.7rem" }} />
                <select value={editAssetType} onChange={(e) => setEditAssetType(e.target.value)} style={{ fontSize: "0.8rem", padding: "0.5rem 0.7rem" }}>
                  {ASSET_TYPES.map((t) => <option key={t} value={t}>{t.replace("_", " ")}</option>)}
                </select>
                <div style={{ display: "flex", gap: "0.4rem" }}>
                  <button className="btn-primary" onClick={() => saveEditAsset(a.id)} style={{ padding: "0.4rem 0.7rem", fontSize: "0.78rem", display: "flex", alignItems: "center", gap: "0.3rem" }}>
                    <Check size={13} /> Save
                  </button>
                  <button className="btn-ghost" onClick={() => setEditingAssetId(null)} style={{ padding: "0.4rem 0.7rem", fontSize: "0.78rem", display: "flex", alignItems: "center", gap: "0.3rem" }}>
                    <X size={13} /> Cancel
                  </button>
                </div>
              </div>
              ...
              <>
                ...
                <a href={a.url} target="_blank" rel="noreferrer" style={{ display: "block" }}>
                  ...
                  <p style={{ fontSize: "0.85rem", marginTop: "0.6rem", paddingRight: "2.5rem" }}>{a.fileName}</p>
