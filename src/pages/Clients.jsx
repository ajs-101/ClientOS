...
export default function Clients() {
  ...
  useEffect(() => {
    if (!activeOrg) return;
    const q = query(collection(db, "clients"), where("orgId", "==", activeOrg));
  ...
  async function handleRiskCheck() {
    ...
    try {
      ...
      setRiskSummary(
        "Couldn't reach the risk checker — try again in a moment.",
      );
    }
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          ...
          />
          <input
            placeholder="Industry (e.g. Legal, Medical)"
            value={industry}
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <span
            style={{
              fontSize: "0.9rem",
              color: riskSummary ? "var(--text-primary)" : "var(--text-muted)",
            }}
          >
            {riskSummary ||
              "Check which clients have deadlines at risk this week."}
          </span>
        </div>
        <button
          className="btn-ghost"
